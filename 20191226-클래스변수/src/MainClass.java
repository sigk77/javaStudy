 ...
 * 					    ~e , ~DTD
 * 		============
 * 			변수 ===> 배열  ==> 구조체(자바에는 x) ==> 클래스 ==> 파일 ==> RDBMS(오라클)
 * 			=================================================== 데이터베이스
 ...
 * 			==========================
 * 			생성자 함수 : 전역변수의 초기화 ==> 한번만 호출 , 객체생성 될 때 처음으로 호출
 * 			윈도우 , 네트워크 , 데이터베이스
 * 			클래스명과 동일 , 리턴형이 없다
...
(주연) 이병헌, 하정우, 마동석, 전혜진, 수지
	...
	class MovieVO
	 ...
	 => int a
	 	douvle d
	 	=============== 컴퓨터가 메모리 크기 인식
	 	movie vo malloc(sozeOf(MovieVO))
	 	======== 메모리 크기를 알 수 없다
	 		메모리 크기 확인 => 메모리를 생성
	 		====================== new
 */
class MovieVO{
	// intstance변수 ==> 메모리에 저장되는 시점 ==> new(새로운 메모리를 만들 때  => 따로 메모리 생성) 
	String title;
	double score;
	String genre;
	String regdate;
